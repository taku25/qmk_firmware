#
#  local_features.mk contains post-processing rules for the Accent keyboard.
#
...
define ACCENT_CUSTOMISE_MSG
  $(info Accent Spacific Build Options)
...
define ACCENT_HELP_MSG
  $(info Accent keyboard convenient command line option)
  $(info -    make ACCENT=<options> accent:<keymap>)
  $(info -    option= oled | core-oled | local-oled | no-oled )
  $(info -            back | no-back   | under      | na   | no-ani )
  $(info -             ios | sc        | split-common | scan | verbose)
  $(info -    eg.)
  $(info -      make ACCENT=no-oled       accent:<keymap>)
  $(info -      make ACCENT=oled,no-back  accent:<keymap>)
  $(info -      make ACCENT=oled,under    accent:<keymap>)
  $(info -      make ACCENT=oled,back,na  accent:<keymap>)
  $(info -      make ACCENT=oled,back,ios accent:<keymap>)
  $(info )
endef

  ifneq ($(strip $(ACCENT)),)
    define ACCENT_OPTION_PARSE
    ...
    endef # end of ACCENT_OPTION_PARSE

    COMMA=,
    $(eval $(foreach A_OPTION_NAME,$(subst $(COMMA), ,$(ACCENT)),  \
        $(call ACCENT_OPTION_PARSE,$(A_OPTION_NAME))))

    ifeq ($(strip $(HELP)),on)
       $(eval $(call ACCENT_HELP_MSG))
       $(error )
    endif
    SHOW_ACCENT_OPTIONS = yes
  endif

ifneq ($(strip $(SPLIT_KEYBOARD)), yes)
  SRC += local_drivers/serial.c
  KEYBOARD_PATHS += $(ACCENT_TOP_DIR)/local_drivers
...
########
# convert Accent-specific options (that represent combinations of standard options)
...
ifeq ($(strip $(OLED_ENABLE)), yes)
    ifeq ($(strip $(OLED_SELECT)),core)
        ...
        ifeq ($(strip $(LOCAL_GLCDFONT)), yes)
           OPT_DEFS += -DOLED_FONT_H=\<accentfont.h\>
        else
           OPT_DEFS += -DOLED_FONT_H=\"common/glcdfont.c\"
        endif
    else
        ifeq ($(strip $(SPLIT_KEYBOARD)), yes)
            $(info accent/rev1: The following combinations are not supported.)
...
ifneq ($(strip $(SHOW_ACCENT_OPTIONS)),)
  $(eval $(call ACCENT_CUSTOMISE_MSG))
